...
class AbstractParser(ABC):
    ...
    def _traverse(self, code: bytes, tree: Tree):
        """
        ...
        while len(stack):
            ...
            if current.type != tree.root_node.type and len(current.children) == 0:
                ...
                tokens.append((token.decode("utf8"), annotation, block_annotation))
    ...
    @staticmethod
    def _extract_token_annotation(code: bytes, node) -> (bytes, str):
        """
...
def tokenize_code(node, blob: str, nodes_to_exclude: Optional[Set] = None) -> List:
    ...
    return [match_from_span(token, blob) for token in tokens if
