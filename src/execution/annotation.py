...
class AnnotationExecution(ExecutionBase):
    def __init__(
        ...
        self.version_strategy = (
            version_strategy if version_strategy else LatestVersionStrategy()
        )
    ...
    def run(self, project: Project) -> Project:
        ...
        for version in versions:
            ...
            version = self.version_builder.build_version(
                project.dir_path, project.languages, version
            )
            ...
            if self.package_annotation_pipeline:
                project, version = self.package_annotation_pipeline.run(
                    project, version
                )

            if self.project_annotation_pipeline:
                project, version = self.project_annotation_pipeline.run(
                    project, version
                )
