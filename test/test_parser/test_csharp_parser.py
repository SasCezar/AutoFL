...
class TestPythonParser(unittest.TestCase):
    def setUp(self) -> None:
        ...
        file_path = Path(self.cfg.test_data_path).joinpath(Path('parser/DetermineParameterTypeHelper.cs'))
        content = self.load_file(file_path)
        self.file = File(path=file_path, language=Extension.c_sharp.name, content=content)
