...
class TestPythonParser(unittest.TestCase):
    def setUp(self) -> None:
        ...
        file_path = Path(self.cfg.test_data_path).joinpath(
            Path("parser/DetermineParameterTypeHelper.cs")
        )
        content = self.load_file(file_path)
        rel_path = str(file_path.relative_to(self.cfg.test_data_path))
        self.file = File(
            path=rel_path, language=Extension.c_sharp.name, content=content
        )
