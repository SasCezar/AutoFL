...
class TestAnnotationExecution(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.annotators: List[Annotator] = instantiate_annotators(
            self.cfg.annotator.annotators, self.taxonomy
        )

        self.file_annotation_pipeline = FileAnnotationPipeline(
            self.annotators, self.ensemble, self.taxonomy
        )
        self.package_annotation_pipeline = PackageAnnotationPipeline()
        self.project_annotation_pipeline = ProjectAnnotationPipeline()

        self.identifier_extraction = IdentifierExtractionPipeline(
            self.cfg.languages_library
        )
        self.version_strategy: VersionStrategyBase = instantiate(
            self.cfg.version_strategy
        )
        self.vcs = VCS()

        self.execution = AnnotationExecution(
            self.identifier_extraction,
            self.file_annotation_pipeline,
            self.package_annotation_pipeline,
